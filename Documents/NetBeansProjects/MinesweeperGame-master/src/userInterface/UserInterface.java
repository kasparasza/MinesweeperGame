...
public class UserInterface {
    ...
    // !!! gal reikia ir opcijos - unflag / bet gal uztenka klaustuko
    // kazkodel "difficulty 1" kartasi duoda daugiau minu nei turetu
    public void makeAMove() {
        // a move is made
        // if true - there was no explosion
        if (this.field.performActionOnCell(readUserInput())) {
            System.out.println(this.field);

            // if all mines were found - the game has to end with a victory
            if (this.field.allMinesFound()) {
                System.out.println("");
            ...
            // if the number of flagged cells is equal to the number of mines -
            // at least one of the flags is in a wrong place
            if (this.field.countMinesRemaining() == 0) {
                informAboutWrongFlags();
            }

            // proceed to a further move
            makeAMove();
        } // if false - the game has to end with a loss
        else {
            System.out.println(this.field);
            System.out.println("");
    ...
    private void informAboutWrongFlags() {
        System.out.println("");
        System.out.println("Check your flags.");
        System.out.println("At least one of them is in a wrong place.");
        System.out.println("");
    }
